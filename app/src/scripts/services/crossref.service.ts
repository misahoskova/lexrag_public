...
export async function detectAllLinks(lawId: number): Promise<void> {
  ...
  for (const row of rows) {
    ...
    if (looksLikeLegalText(s)) {
      ...
      let mPriloha: RegExpExecArray | null;
      while ((mPriloha = rePriloha.exec(s)) !== null) {
        const spanStart = mPriloha.index ?? 0;
        const spanEnd = rePriloha.lastIndex;
        const rawText = s.slice(spanStart, spanEnd);

        const addr = parsePrilohaFromMatch(mPriloha);
        const normalized = normalizePrilohaRef(mPriloha);

        let targetNodeId: string | null = null;
        if (addr.priloha) {
          targetNodeId = await resolveNodeId(lawId, addr);
        }
        ...
        await upsertCrossref({
          ...
          normalized,
          targetNodeId,
          confidence: targetNodeId ? 100 : 70,
          note: 'Odkaz na přílohu detekován',
        });
        totalPrilohaRefs++;
      }
    }
  ...
  console.log(`✅ Odkazy: uloženo ${totalPrilohaRefs} odkazů na přílohy.`);
}
