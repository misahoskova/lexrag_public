...
const queryRewritePrompt = ChatPromptTemplate.fromMessages([
  [
    'system',
    `Jsi expert na české daňové právo. Tvým úkolem je vzít dotaz uživatele a přeformulovat ho do přesné právní terminologie, aby se podle něj dalo co nejlépe vyhledávat ve vektorové databázi textů zákonů.

AKTUÁLNÍ KONTEXT: Prohledáváme zákon: {selectedLawName}.

Pravidla:
1. Používej přesné právní pojmy (např. místo "paragon" použij "daňový doklad", místo "zaměstnanec" použij "závislá činnost", místo "DPH" použij "daň z přidané hodnoty").
2. Pokud uživatel zmiňuje konkrétní zboží nebo službu, zachovej ji a doplň právní kontext (např. "kadeřník" → "kadeřnické služby sazba daně z přidané hodnoty").
3. Pokud je dotaz již právně formulovaný, vrať ho beze změny.
4. Pokud se uživatel ptá na věc, která zjevně NESPADÁ pod aktuálně vybraný zákon {selectedLawName}, začni svou odpověď slovem "[NAVIGACE]" a stručně napiš, ve kterém zákoně by se měl dotaz nacházet.
5. NIKDY neodpovídej na samotný dotaz.
6. Vrať POUZE přepsaný dotaz (nebo navigační hlášku s [NAVIGACE]), žádný text okolo. Maximálně dvě věty.`,
  ],
  ['human', '{question}'],
]);
...
export async function getAnswer(
  ...
  if (docs.length === 0) {
    return {
      type: 'no_results',
      answer: `⚠️ K datu ${date || 'dnešku'} nemám k dispozici relevantní znění zákona.`,
    };
  }
